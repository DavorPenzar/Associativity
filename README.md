...
The format of the [*CSV* files](http://en.wikipedia.org/wiki/Comma-separated_values) supported by the implemented parser is extensively described in the documentation of [`TableReader` class](app/src/main/java/com/example/associativity/TableReader.kt), more specifically [`TableReader.readCSV`](app/src/main/java/com/example/associativity/TableReader.kt#L308) and [`TableReader.escapeExpression`](app/src/main/java/com/example/associativity/TableReader.kt#L221) methods, but essentially comes down to the following:
...
13. Although this is standard [*CSV*](http://en.wikipedia.org/wiki/Comma-separated_values) practice, it is worth mentioning that adding extra separators at the beginning and/or the end of a row will result in empty cells at the beginning and/or the end (I am not saying cases where this is the desired result do not exist&mdash;such cases are even mentioned in [**Game Table Format**](#game-table-format)). The first cell of a row starts **immediately at the beginning of the row's line**, and the last one ends **at the very end of the row's line** (leading and trailing whitespaces will be ignored in cases explained above).
...
In both formats shown commas at the end of rows 2 &ndash; 5 may be omitted, but, if a final comma is omitted in at least one of the rows, final commas must be omitted in all of them. All 4 formats are supported and will result in the same game table (remember, leading and trailing whitespaces are **ignored**).
...
**If you only want to mount your game tables into the app, but do not have the will or knowledge to build the app yourself**, I am sure you can somehow locate [*app/src/main/assets/game_tables* assets subdirectory](app/src/main/assets/game_tables) on your device and change its contents (adding new game tables, removing the existing ones and so on). Again, as it is mentioned in the [*app/src/main/assets/game_tables* assets subdirectory's README.md file](app/src/main/assets/game_tables/README.md), the subdirectory *0* is intended for custom game tables, while the other subdirectories are intended for game tables being distributed with the app itself. Note that the app is currently under construction and I have not investigated this option yet.
